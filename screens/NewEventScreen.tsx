...
const NewEventComponent = () => {
  ...
  const createEvent = async () => {
    ...
    const newEvent: MicrosoftGraph.Event = {
      ...
      start: {
        dateTime: format(newEventState.startDate, GRAPH_DATETIME_FORMAT),
        timeZone: newEventState.timeZone,
      },
      end: {
        dateTime: format(newEventState.endDate, GRAPH_DATETIME_FORMAT),
        timeZone: newEventState.timeZone,
      },
    };
...
const getDefaultStart = (): Date => {
  ...
  const offset = 30 - (getMinutes(now) % 30);
...
export default class NewEventScreen extends React.Component {
  ...
  disableCreate = () => {
    return (
      ...
      compareAsc(this.state.startDate, this.state.endDate) >= 0
    );
  };
